...
#else
#ifdef DEBUG_TRACE
#define LOGV(format, args...) do {\
fprintf(stdout, \
"ATLIB_VERBOSE [%s, %d] " format "\n", __FUNCTION__, __LINE__, ##args); }\
 while (0)
#define LOGD(format, args...) do {\
fprintf(stdout, \
"ATLIB_DEBUG [%s, %d] " format "\n", __FUNCTION__, __LINE__, ##args); }\
 while (0)
#define LOGI(format, args...) do {\
fprintf(stdout, \
"ATLIB_INFO [%s, %d] " format "\n", __FUNCTION__, __LINE__, ##args); }\
 while (0)
#define LOGW(format, args...) do {\
fprintf(stdout, \
"ATLIB_WARNING [%s, %d] " format "\n", __FUNCTION__, __LINE__, ##args); }\
 while (0)
#define LOGE(format, args...) do {\
fprintf(stdout, \
"ATLIB_ERROR [%s, %d] " format "\n", __FUNCTION__, __LINE__, ##args); }\
 while (0)
#else
#define LOGV(format, args...) do {} while (0)
#define LOGD(format, args...) do {} while (0)
#define LOGI(format, args...) do {} while (0)
#define LOGW(format, args...) do {} while (0)
...
static void processLine(const char *line)
{
    ...
    if (ac->response == NULL || ac->response->finalResponse)
        /* No command pending. */
...
static const char *readline()
{
	...
	/* 1 s timeout to allow escape sequence - typically when we will
	 * shutdown the AT channel
	 */
        err = poll(pfds, 2, 1000);

	/* Timeout, we check exit condition and... */
	if (ac->readerClosed)
	    return NULL;
	/* ... continue. */
	if (err == 0)
...
/**
 * Sets libat's file descr handle to -1.
 * Note that this should only be used in connection with at_close().
 * Disabling the file descr during regular execution will cause libat to
 * fail. The obvious use case for this is to keep the AT channel alive after
 * the libat thread exits.
 */
void at_detach_filedesc()
{
	struct atcontext *ac = getAtContext();

	LOGD("at_detach_filedesc");
	pthread_mutex_lock(&ac->commandmutex);
	ac->fd = -1;
	pthread_mutex_unlock(&ac->commandmutex);
}
